...
void lcd_rotatingBanner(char fullMessage[]){

    char message[] = "                    "; //20 Spaces for LCD Width

    while(1){
        int i;
        for(i = 0; i < strlen(fullMessage); i++){ //Loop for full message
            lcd_rotateLeft(message, fullMessage[i]);
        }

        for(i = 0; i < LCD_WIDTH; i++){
            lcd_rotateLeft(message, ' ');
        }
    }
}

void lcd_rotateLeft(char *message, char newChar){
    int i;
    for(i = 1; i < strlen(message); i++){
        message[i - 1] = message[i];
    }
    message[strlen(message) - 1] = newChar;
    lcd_printf(message);
    timer_waitMillis(300); //0.3 Second delay
}
